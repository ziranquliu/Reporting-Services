...
path = path.Replace("{" + "CommentId" + "}", ApiClient.ParameterToString(commentId));
        ...
        /// <param name="_file">The file contents.</param> 
        /// <returns>MobileReport</returns>            
        public MobileReport UploadMobileReport (string id, System.IO.Stream _file)
        {
            
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling UploadMobileReport");
            
            // verify the required parameter '_file' is set
            if (_file == null) throw new ApiException(400, "Missing required parameter '_file' when calling UploadMobileReport");
            
    
            var path = "/MobileReports({Id})/Model.Upload";
            path = path.Replace("{format}", "json");
            path = path.Replace("{" + "Id" + "}", ApiClient.ParameterToString(id));
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling UploadMobileReport: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling UploadMobileReport: " + response.ErrorMessage, response.ErrorMessage);
