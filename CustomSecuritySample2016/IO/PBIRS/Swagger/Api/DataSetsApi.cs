...
namespace IO.PBIRS.Swagger.Api
{
    ...
    public interface IDataSetsApi
    {
        ...
        /// </summary>
        /// <param name="id">The key (GUID or path) that uniquely identifies the object. GUID example: 01234567-89ab-cdef-0123-456789abcdef path example: path&#x3D;&#39;/folder1/folder2/item&#39; If the path itself contains single quote, it should be escaped - add another single quote.</param>
        /// <returns>ODataDataSetRows</returns>
...
path = path.Replace("{" + "DataSourceId" + "}", ApiClient.ParameterToString(dataSourceId));
        ...
        /// <param name="id">The key (GUID or path) that uniquely identifies the object. GUID example: 01234567-89ab-cdef-0123-456789abcdef path example: path&#x3D;&#39;/folder1/folder2/item&#39; If the path itself contains single quote, it should be escaped - add another single quote.</param> 
        /// <returns>ODataDataSetRows</returns>            
        public ODataDataSetRows GetDataSetODataFeed (string id)
        {
            
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling GetDataSetODataFeed");
            
    
            var path = "/DataSets({Id})/Data";
            path = path.Replace("{format}", "json");
            path = path.Replace("{" + "Id" + "}", ApiClient.ParameterToString(id));
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetDataSetODataFeed: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetDataSetODataFeed: " + response.ErrorMessage, response.ErrorMessage);
    
            return (ODataDataSetRows) ApiClient.Deserialize(response.Content, typeof(ODataDataSetRows), response.Headers);
        }
...
path = path.Replace("{" + "DataSourceId" + "}", ApiClient.ParameterToString(dataSourceId));
        ...
        /// <param name="_file">The file contents.</param> 
        /// <returns>DataSet</returns>            
        public DataSet UploadDataSet (string id, System.IO.Stream _file)
        {
            
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling UploadDataSet");
            
            // verify the required parameter '_file' is set
            if (_file == null) throw new ApiException(400, "Missing required parameter '_file' when calling UploadDataSet");
            
    
            var path = "/DataSets({Id})/Model.Upload";
            path = path.Replace("{format}", "json");
            path = path.Replace("{" + "Id" + "}", ApiClient.ParameterToString(id));
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling UploadDataSet: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling UploadDataSet: " + response.ErrorMessage, response.ErrorMessage);
