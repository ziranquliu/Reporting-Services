...
namespace IO.PBIRS.Swagger.Api
{
    ...
    public interface ICacheRefreshPlansApi
    {
        ...
        /// </summary>
        /// <param name="id">The GUID that uniquely identifies the CacheRefreshPlan. GUID example: 01234567-89ab-cdef-0123-456789abcdef</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="id">The GUID that uniquely identifies the CacheRefreshPlan. GUID example: 01234567-89ab-cdef-0123-456789abcdef</param>
        /// <returns>ODataSubscriptionHistory</returns>
    ...
    public class CacheRefreshPlansApi : ICacheRefreshPlansApi
    {
        ...
        /// <param name="id">The GUID that uniquely identifies the CacheRefreshPlan. GUID example: 01234567-89ab-cdef-0123-456789abcdef</param> 
        /// <returns>ODataSubscriptionHistory</returns>            
        public ODataSubscriptionHistory GetCacheRefreshPlanHistory (string id)
        {
            
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling GetCacheRefreshPlanHistory");
            
    
            var path = "/CacheRefreshPlans({Id})/History";
            path = path.Replace("{format}", "json");
            path = path.Replace("{" + "Id" + "}", ApiClient.ParameterToString(id));
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetCacheRefreshPlanHistory: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetCacheRefreshPlanHistory: " + response.ErrorMessage, response.ErrorMessage);
    
            return (ODataSubscriptionHistory) ApiClient.Deserialize(response.Content, typeof(ODataSubscriptionHistory), response.Headers);
        }
